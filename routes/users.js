...
router.post('/registered',
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            bcrypt.hash(req.sanitize(req.body.password), saltRounds, (err, hashedPassword) => {
        ...
        `
                ...
                db.query(sqlQuery, newRecord, (err, result) => {
                    if (err) return res.send(err)
                    res.send(`Hello ${req.sanitize(req.body.first)} ${req.sanitize(req.body.last)} you are now registered!
                    We will send an email to you at ${req.sanitize(req.body.email)}. Your password is:
                    ${req.sanitize(req.body.password)} and your hashed password is: ${hashedPassword}
                `)
                });
            })
        };
...
router.post("/loggedin", (req, res, next) => {
    db.query("SELECT hashedPassword FROM userdata WHERE username = ?", req.sanitize(req.body.username), (err, result) => {
        if(err) return next(err)
        if(!result[0]) return res.send("User not found")

        bcrypt.compare(req.sanitize(req.body.password), result[0].hashedPassword, (err, equal) => {
            if (err) return next(err)
            ...
            db.query(query, [outcome, req.sanitize(req.body.username)], (err, result) => { return })
            if (equal) {
